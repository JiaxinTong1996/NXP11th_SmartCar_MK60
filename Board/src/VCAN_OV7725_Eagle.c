...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTB_B0_IN, (void *)ov7725_eagle_img_buff, PTB10, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
    ...
    port_init(PTB10, ALT1 | DMA_FALLING | PULLDOWN );         //PCLK

    port_init(PTB17, ALT1 | IRQ_RISING  | PULLDOWN | PF);     //场中断，上拉，上降沿触发中断，带滤波
...
void ov7725_eagle_vsync(void)
{
    ...
    if(ov7725_eagle_img_flag == IMG_START)                   //需要开始采集图像
    {
        ...
        PORTB_ISFR = 1 <<  PT10;            //清空PCLK标志位  PTB10

        DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求
        PORTB_ISFR = 1 <<  PT10;            //清空PCLK标志位
