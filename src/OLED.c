...
************************************************************************/
  ...
  #define OLED_SCL  PTB11_OUT
  #define OLED_SDA  PTB16_OUT
  #define OLED_RST  PTD15_OUT
  #define OLED_DC   PTD14_OUT
...
void OLED_Init(void)
{
         ...
         gpio_init (PTD14 , 1,1);
         gpio_init (PTB16 ,1,1);
         gpio_init (PTD4 , 1,0); //CS引脚，没使用时软件置低
         gpio_init (PTD15 , 1,0); //LCD_RST
...
void servPWMDutyDis()
{
  uint8 table[1][5];
  int a;
  if(servPWMDuty>=1070&&servPWMDuty<=1500)
  {
    a=servPWMDuty;
    //table[0]=a;
  }
  sprintf(table[0],"%4d",a);
  OLED_Print(0,0,table[0]);
}
extern int8 centureErr;
void centureErrDis()
{
  int8 table1[1][5];
  int8 tmp;
 if(centureErr>=0)
  ...
  sprintf(table1[0],"%3d",tmp);
  OLED_Print(0,2,table1[0]);  
}
...
void DutyDis()
{
  ...
  uint32 tmp1;
  uint32 tmp2;
  tmp1=speed_out_set_str;
  tmp2=speed_out_set;
  sprintf(table2[0],"%3d",tmp1);
  sprintf(table3[0],"%3d",tmp2);
  OLED_Print(0,4,table2[0]);  
  OLED_Print(0,6,table3[0]);
}
